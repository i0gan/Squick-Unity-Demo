...
namespace BM
{
    public class BuildAssets : EditorWindow
    {
        ...
        /// 分包配置文件资源目录
        /// </summary>
        public static string AssetLoadTablePath = "Assets/Core/BundleMaster/Editor/BundleMasterEditor/BuildSettings/AssetLoadTable.asset";
        ...
        public static void CreateSingleSetting()
        {
            ...
            AssetDatabase.CreateAsset(assetsLoadSetting, "Assets/Core/BundleMaster/Editor/BundleMasterEditor/BuildSettings/AssetsLoadSetting.asset");
        }


        [MenuItem("Tools/BuildAsset/Build Android")]
        public static void BuildAndroid()
        {
            ...
            {
                Debug.Log("编译当前版本 <= 之前版本，请重新修改配置文件");
            }

            PlayerSettings.Android.keystoreName = androidBuildConfigAsset.keystorePath;
            PlayerSettings.Android.keystorePass = androidBuildConfigAsset.keystorePassword;
            PlayerSettings.Android.keyaliasName = androidBuildConfigAsset.keyaliasName;
            PlayerSettings.Android.keyaliasPass = androidBuildConfigAsset.keyaliasPassword;
            PlayerSettings.Android.bundleVersionCode = androidBuildConfigAsset.bundleVersionCode;
            //开始构建前剔除多余场景
            ...
            string outputPath = Path.Combine(assetLoadTable.BuildBundlePath, "application.apk");
            ...
            JObject jdata = new JObject
            {
                ...
                { "Version", float.Parse(Application.version) },
                { "Date", "none" }
            };

            FileStream fs = new FileStream(Path.Combine(assetLoadTable.BuildBundlePath, "version"), FileMode.OpenOrCreate, FileAccess.ReadWrite);
            StreamWriter sw = new StreamWriter(fs);
            sw.Write(jdata.ToString());
            sw.Flush();
            sw.Close();
            Debug.Log("编译安卓结束");
        }
        ...
        private static void Build(AssetLoadTable assetLoadTable, AssetsLoadSetting assetsLoadSetting, HashSet<string> assetLoadPath)
        {
            ...
            for (int i = 0; i < sceneAssets.Length; i++)
            {
                ...
                {
                    files.Add(scenePath);
                    Debug.Log($"Compile Assets bundle ：{scenePath}");
                }
            }
            //分析所有需要主动加载的资源
