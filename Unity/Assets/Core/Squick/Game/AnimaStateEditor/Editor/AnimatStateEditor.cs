...
namespace SquickProtocol
{
	...
	[CustomEditor(typeof(AnimatStateController))]
	public class AnimatStateEditor : Editor
	{
		#region 数据成员
		private Animator mAnimator;
		private AnimatStateController mASC;
		private AnimaStateData mData;
        ...
        //预览专用
        // 当前预览的动画
        private AnimaStateType mPlayType = AnimaStateType.NONE;
        //手动预览的动画
        private AnimaStateType mManulPlayType = AnimaStateType.NONE;
        /// 上一次系统时间
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (mData)
            {
                ...
                mData.DefaultAnimationType = (AnimaStateType)EditorGUILayout.EnumPopup("DefaultAnima:", mData.DefaultAnimationType);
            ...
            else
            {
                if (GUILayout.Button("Fix All anims"))
                {
                    ...
                    mData = ScriptableObject.CreateInstance<AnimaStateData>();
                    UnityEditor.AssetDatabase.CreateAsset(mData, strPath + ".asset");
                    UnityEditor.AssetDatabase.SaveAssets();
                    UnityEditor.AssetDatabase.Refresh();
					
                    mASC.mxSkillData = UnityEditor.AssetDatabase.LoadAssetAtPath<AnimaStateData>(strPath + ".asset");
                }
        ...
        void FixAllAnims()
		{
            ...
            for (int i = 0; i < mData.AnimationSkillList.Count; i++)
            {
				...
				if (anim == null)
                {
                    ...
                    for (int index = 0; index < mData.AnimationSkillList.Count; index++)
                    {
                        ...
                        if (tempAss.Type == AnimaStateType.Idle)
                        {
                ...
                else
                {
					...
					List<UnityEngine.AnimationEvent> animationEvents = new List<UnityEngine.AnimationEvent>();
            ...
            AnimatorState runState = intArray2[(int)AnimaStateType.Run];
            AnimatorState IdleState = intArray2[(int)AnimaStateType.Idle];
            ...
            */
			...
			//string prefabPath = "Assets/HotUpdate/Prefab/Object/" + mASC.gameObject.name + ".prefab";
        ...
        void DrawAnimation()
        {
            ...
            for (int i = 0; i < mData.AnimationSkillList.Count; i++)
            {
                ...
                AnimationSkillStruct _ass = mData.AnimationSkillList[i];
                if (_ass.Type == AnimaStateType.NONE)
                {
                    continue;
                }
                ...
                if (_ass.IsFoldout)
                {
                    ...
                    _ass.NextType = (AnimaStateType)EditorGUILayout.EnumPopup(_ass.NextType);
		...
		private void update(float delta)
        {
            ...
            // 设置回放的时间位置
            if (mManulPlayType != AnimaStateType.NONE)
            {
                manualUpdate(mManulPlayType, m_CurrentTime);
            }
            else
            {
                if (mPlayType != AnimaStateType.NONE)
                {
        ...
        /// </summary>
        private void manualUpdate(AnimaStateType stateType, float time)
        {
            if (mAnimator != null && stateType != AnimaStateType.NONE)
            {
