...
namespace Uquick.Editor
{
    public static partial class SerializeILTypeInstance
    {
        ...
        {
            //如果JBehaviour
            var jBehaviourType = InitUquick.Appdomain.LoadedTypes["Uquick.Core.JBehaviour"];
    ...
    public static partial class SerializeILTypeInstance
    {
            ...
            string name)
        {
            ...
            if (type.ReflectionType.ToString().Contains("BindableProperty") && obj != null)
            {
                ...
                {
                    //可更改
                    var data = Uquick.Core.Tools.ConvertSimpleType(EditorGUILayout.TextField(objName, val?.ToString()),
                        genericType);
                    if (data != null) //尝试更改
                    {
