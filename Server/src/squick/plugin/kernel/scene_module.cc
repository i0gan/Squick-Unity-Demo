	...
	if (pSceneInfo)
	{
		...
		if (!pSceneInfo->GetElement(nNewGroupID))
		{
			...
			if (pGroupInfo)
			{
				...
				SQUICK_SHARE_PTR<IPropertyManager> pStaticClassPropertyManager = m_pClassModule->GetClassPropertyManager(SquickProtocol::Group::ThisName());
				SQUICK_SHARE_PTR<IRecordManager> pStaticClassRecordManager = m_pClassModule->GetClassRecordManager(SquickProtocol::Group::ThisName());
				if (pStaticClassPropertyManager && pStaticClassRecordManager)
				{
					...
					while (pStaticConfigPropertyInfo)
					{
						...
						PROPERTY_EVENT_FUNCTOR functor = std::bind(&SceneModule::OnScenePropertyCommonEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5);
					...
					while (pConfigRecordInfo)
					{
						...
						RECORD_EVENT_FUNCTOR functor = std::bind(&SceneModule::OnSceneRecordCommonEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
				...
				int sceneType = m_pElementModule->GetPropertyInt32(std::to_string(sceneID), SquickProtocol::Scene::Type());
...
bool SceneModule::RequestEnterScene(const Guid & self, const int sceneID, const int groupID, const int type, const Vector3& pos, const DataList & argList)
{
	...
	const int nNowSceneID = m_pKernelModule->GetPropertyInt32(self, SquickProtocol::Player::SceneID());
	const int nNowGroupID = m_pKernelModule->GetPropertyInt32(self, SquickProtocol::Player::GroupID());
...
bool SceneModule::LeaveSceneGroup(const Guid & self)
{
	...
	if (pObject)
	{
		int nOldSceneID = pObject->GetPropertyInt32(SquickProtocol::Scene::SceneID());
		int nOldGroupID = pObject->GetPropertyInt32(SquickProtocol::Scene::GroupID());
		if (nOldGroupID <= 0)
		{
		...
		const Vector3& lastPos = m_pKernelModule->GetPropertyVector3(self, SquickProtocol::IObject::Position());
		...
		{
			pObject->SetPropertyInt(SquickProtocol::Scene::GroupID(), 0);
			/////////
...
bool SceneModule::CreateSceneNPC(const int sceneID, const int groupID)
{
	return CreateSceneNPC(sceneID, groupID, DataList::Empty());
}
...
bool SceneModule::CreateSceneNPC(const int sceneID, const int groupID, const DataList& argList)
{
	...
	for (; pResource; pResource = pSceneInfo->mtSceneResourceConfig.Next())
	{
		...
		{
			DataList arg;
			arg << SquickProtocol::IObject::Position() << pResource->vSeedPos;
...
int SceneModule::GetPropertyInt32(const int scene, const int group, const std::string & propertyName)
{
...
bool SceneModule::SetRecordFloat(const int scene, const int group, const std::string & recordName, const int row, const std::string & colTag, const double value)
{
	SQUICK_SHARE_PTR<IRecord> xRecord = FindRecord(scene, group, recordName);
	if (xRecord)
	{
...
bool SceneModule::SetRecordString(const int scene, const int group, const std::string & recordName, const int row, const std::string & colTag, const std::string & value)
{
	SQUICK_SHARE_PTR<IRecord> xRecord = FindRecord(scene, group, recordName);
	if (xRecord)
	{
...
bool SceneModule::SetRecordObject(const int scene, const int group, const std::string & recordName, const int row, const std::string & colTag, const Guid & value)
{
	SQUICK_SHARE_PTR<IRecord> xRecord = FindRecord(scene, group, recordName);
	if (xRecord)
	{
...
bool SceneModule::SetRecordVector2(const int scene, const int group, const std::string & recordName, const int row, const std::string & colTag, const Vector2 & value)
{
	SQUICK_SHARE_PTR<IRecord> xRecord = FindRecord(scene, group, recordName);
	if (xRecord)
	{
...
bool SceneModule::SetRecordVector3(const int scene, const int group, const std::string & recordName, const int row, const std::string & colTag, const Vector3 & value)
{
	SQUICK_SHARE_PTR<IRecord> xRecord = FindRecord(scene, group, recordName);
	if (xRecord)
	{
...
bool SceneModule::SwitchScene(const Guid& self, const int nTargetSceneID, const int nTargetGroupID, const int type, const Vector3 v, const float fOrient, const DataList& arg)
{
	SQUICK_SHARE_PTR<IObject> pObject = m_pKernelModule->GetObject(self);
	if (pObject)
	{
		...
		//if (nTargetSceneID != nOldSceneID)
		{
			...
			pObject->SetPropertyInt(SquickProtocol::Scene::SceneID(), nTargetSceneID);
		...
		pObject->SetPropertyInt(SquickProtocol::Scene::GroupID(), nTargetGroupID);


		const Guid newCell = m_pCellModule->ComputeCellID(v);
		OnMoveCellEvent(self, nTargetGroupID, nTargetGroupID, Guid(), newCell);

		pObject->SetPropertyVector3(SquickProtocol::IObject::MoveTo(), v);
...
int SceneModule::OnPropertyCommonEvent(const Guid & self, const std::string & propertyName, const SquickData & oldVar, const SquickData & newVar, const INT64 reason)
{
	...
	if (className == SquickProtocol::Player::ThisName())
	{
		...
		if (SquickProtocol::Player::GroupID() == propertyName)
		{
			OnPlayerGroupEvent(self, propertyName, oldVar, newVar);
			return 0;
		}

		if (SquickProtocol::Player::SceneID() == propertyName)
		{
...
int SceneModule::OnRecordCommonEvent(const Guid & self, const RECORD_EVENT_DATA & eventData, const SquickData & oldVar, const SquickData & newVar)
{
	...
	int nObjectGroupID = m_pKernelModule->GetPropertyInt32(self, SquickProtocol::Player::GroupID());
...
int SceneModule::OnClassCommonEvent(const Guid & self, const std::string & className, const CLASS_OBJECT_EVENT classEvent, const DataList & var)
{
	...
	{
		const int nObjectSceneID = m_pKernelModule->GetPropertyInt32(self, SquickProtocol::IObject::SceneID());
		const int nObjectGroupID = m_pKernelModule->GetPropertyInt32(self, SquickProtocol::IObject::GroupID());
		...
		const Vector3& pos = m_pKernelModule->GetPropertyVector3(self, SquickProtocol::Player::Position());
...
int SceneModule::OnPlayerGroupEvent(const Guid & self, const std::string & propertyName, const SquickData & oldVar, const SquickData & newVar)
{
	...
	const Vector3 position = m_pKernelModule->GetPropertyVector3(self, SquickProtocol::IObject::Position());
...
{
	const int nObjectContainerID = m_pKernelModule->GetPropertyInt32(self, SquickProtocol::IObject::SceneID());
	const int nObjectGroupID = m_pKernelModule->GetPropertyInt32(self, SquickProtocol::IObject::GroupID());
	const Vector3& position = m_pKernelModule->GetPropertyVector3(self, SquickProtocol::IObject::Position());
