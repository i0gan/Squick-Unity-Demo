...
SQUICK_SHARE_PTR<IObject> KernelModule::CreateObject(const Guid& self, const int sceneID, const int groupID, const std::string& className, const std::string& configIndex, const DataList& arg)
{
		...
		m_pThreadPoolModule->DoAsyncTask(Guid(), "",
			...
			[=](ThreadTask& task) -> void
			{
				//no data--main thread
				{
					...
					pObject->SetPropertyString(SquickProtocol::IObject::ConfigID(), configIndex);
					pObject->SetPropertyString(SquickProtocol::IObject::ClassName(), className);
					pObject->SetPropertyInt(SquickProtocol::IObject::SceneID(), sceneID);
					pObject->SetPropertyInt(SquickProtocol::IObject::GroupID(), groupID);
					pObject->SetPropertyVector3(SquickProtocol::IObject::Position(), vRelivePos);

					pContainerInfo->AddObjectToGroup(groupID, ident, className == SquickProtocol::Player::ThisName() ? true : false);
					...
					[=](ThreadTask& task) -> void
					{
						...
						//main thread
						{
							...
							for (int i = 0; i < arg.GetCount() - 1; i += 2)
							{
								...
								if (SquickProtocol::IObject::ConfigID() != propertyName
									&& SquickProtocol::IObject::ClassName() != propertyName
									&& SquickProtocol::IObject::SceneID() != propertyName
									&& SquickProtocol::IObject::ID() != propertyName
									&& SquickProtocol::IObject::GroupID() != propertyName)
								{
							...
							stream << " position: " << pObject->GetPropertyVector3(SquickProtocol::IObject::Position()).ToString();
	...
	else
	{
		...
		//no data--main thread
		{
			...
			pObject->SetPropertyObject(SquickProtocol::IObject::ID(), ident);
			pObject->SetPropertyString(SquickProtocol::IObject::ConfigID(), configIndex);
			pObject->SetPropertyString(SquickProtocol::IObject::ClassName(), className);
			pObject->SetPropertyInt(SquickProtocol::IObject::SceneID(), sceneID);
			pObject->SetPropertyInt(SquickProtocol::IObject::GroupID(), groupID);
...
bool KernelModule::DestroyObject(const Guid& self)
{
	...
	const int sceneID = GetPropertyInt32(self, SquickProtocol::IObject::SceneID());
	const int groupID = GetPropertyInt32(self, SquickProtocol::IObject::GroupID());

	SQUICK_SHARE_PTR<SceneInfo> pContainerInfo = m_pSceneModule->GetElement(sceneID);
	if (pContainerInfo)
	{
		const std::string& className = GetPropertyString(self, SquickProtocol::IObject::ClassName());
...
bool KernelModule::GetGroupObjectList(const int sceneID, const int groupID, const std::string & className, DataList& list, const Guid& noSelf)
{
	...
	if (GetGroupObjectList(sceneID, groupID, xDataList))
	{
		for (int i = 0; i < xDataList.GetCount(); i++)
		{
			...
			if (this->GetPropertyString(xID, SquickProtocol::IObject::ClassName()) == className
				&& xID != noSelf)
			{
				list.AddObject(xID);
			}
...
bool KernelModule::LogInfo(const Guid ident)
{
	...
	if (pObject)
	{
		int sceneID = GetPropertyInt32(ident, SquickProtocol::IObject::SceneID());
		int groupID = GetPropertyInt32(ident, SquickProtocol::IObject::GroupID());
...
int KernelModule::OnPropertyCommonEvent(const Guid& self, const std::string& propertyName, const SquickData& oldVar, const SquickData& newVar, const INT64 reason)
{
	...
	if (xObject)
	{
		if (xObject->GetState() >= CLASS_OBJECT_EVENT::COE_CREATE_HASDATA)
		{
			...
			const std::string& className = xObject->GetPropertyString(SquickProtocol::IObject::ClassName());
