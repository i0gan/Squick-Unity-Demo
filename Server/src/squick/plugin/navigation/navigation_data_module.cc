...
bool NavigationDataModule::AfterStart()
{
   ...
   // public Dictionary<int, Dictionary<Guid, Voxel>> data;
    auto sceneElement = m_pClassModule->GetElement(SquickProtocol::Scene::ThisName());
    ...
    for (auto it = sceneList.begin(); it != sceneList.end(); ++it)
    {
        ...
        const std::string& sceneName = m_pElementModule->GetPropertyString(sceneID, SquickProtocol::Scene::SceneName());
