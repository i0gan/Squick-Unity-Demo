...
bool NoSqlModule::AfterStart()
{
	...
	SQUICK_SHARE_PTR<IClass> xLogicClass = m_pClassModule->GetElement(SquickProtocol::NoSqlServer::ThisName());
	if (xLogicClass)
	{
		...
		for (int i = 0; i < strIdList.size(); ++i)
		{
			...
			const int serverID = m_pElementModule->GetPropertyInt32(strId, SquickProtocol::NoSqlServer::ServerID());
			const int nPort = m_pElementModule->GetPropertyInt32(strId, SquickProtocol::NoSqlServer::Port());
			const std::string& ip = m_pElementModule->GetPropertyString(strId, SquickProtocol::NoSqlServer::IP());
			const std::string& strAuth = m_pElementModule->GetPropertyString(strId, SquickProtocol::NoSqlServer::Auth());
...
bool NoSqlModule::Enable()
{
	return false;
}

bool NoSqlModule::Busy()
{
	return false;
}

bool NoSqlModule::KeepLive()
{
	return false;
}
...
bool NoSqlModule::RemoveConnectSql(const std::string& strID)
{
