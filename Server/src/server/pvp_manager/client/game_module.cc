...
bool GameModule::Start()
{
	...
	m_ServerModule = pPluginManager->FindModule<pvp_manager::server::IServerModule>();
		...
		for (int i = 0; i < strIdList.size(); ++i)
		{
            ...
            if (serverType == SQUICK_SERVER_TYPES::SQUICK_ST_PVP_MANAGER && pPluginManager->GetAppID() == serverID)
            {
                ...
                if (pServerData)
                {
                    ...
                    m_pNetClientModule->SendToServerByPB(nTargetID, SquickStruct::ServerMsgId::PVP_MANAGER_TO_GAME_REGISTERED, xMsg);
