...
bool ServerModule::AfterStart()
{
    ...
    m_pNetModule->AddReceiveCallBack(SquickStruct::REQ_CONNECT_GAME_SERVER, this, &ServerModule::OnReqConnectGameServer);
...
{
    dout << "--- PVP服务器与Game服务器建立连接 " << std::endl;
    Guid nPlayerID;
    SquickStruct::ReqConnectGameServer xMsg;
    ...
    // 验证Token
	bool bRet = m_pSecurityModule->VerifySecurityKey(xMsg.instance_id(), xMsg.security_code());
    ...
    pNetObject->SetAccount(xMsg.instance_id());
    ...
    *xSendMsg.mutable_event_client() = INetModule::StructToProtobuf(pNetObject->GetClientID());
    m_pNetModule->SendMsgPB(SquickStruct::ACK_CONNECT_GAME_SERVER, xSendMsg, sockIndex);
        ...
        if (nGameID > 0)
        {
            ...
            if (!pNetObject->GetUserID().IsNull())
            {
                ...
                *xMsg.mutable_player_id() = INetModule::StructToProtobuf(pNetObject->GetUserID());
                ...
                // 也告知一下Game服务器
				m_pNetClientModule->SendByServerIDWithOutHead(nGameID, SquickStruct::ServerMsgId::REQ_PVP_INSTANCE_DESTROY, msg);
            }
        }
