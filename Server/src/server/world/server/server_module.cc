...
void WorldNet_ServerModule::SynWorldToGame()
{
	...
	while (pServerData)
	{
		if (pServerData->pData->server_state() != SquickStruct::EServerState::EST_MAINTEN
			&& pServerData->pData->server_state() != SquickStruct::EServerState::EST_CRASH)
		{
...
void WorldNet_ServerModule::OnClientDisconnect(const SQUICK_SOCKET nAddress)
{
    ...
    while (pServerData)
    {
        if (nAddress == pServerData->nFD)
        {
			...
			ServerReport(pServerData->pData->server_id(), SquickStruct::EST_CRASH);
            SynGameToProxy();
            break;
        }
...
void WorldNet_ServerModule::ServerReport(int reportServerId, SquickStruct::EServerState serverStatus)
{
