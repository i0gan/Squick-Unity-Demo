		...
		for (int i = 0; i < strIdList.size(); ++i)
		{
			...
			if (serverType == SQUICK_SERVER_TYPES::SQUICK_ST_GAME && pPluginManager->GetAppID() == serverID)
			{
				...
				if (pServerData)
				{
					...
					m_pNetClientModule->SendToServerByPB(nTargetID, SquickStruct::ServerMsgId::GAME_TO_WORLD_REGISTERED, xMsg);
...
bool GameServerToWorldModule::AfterStart()
{
	...
	m_pKernelModule->AddClassCallBack(SquickProtocol::Player::ThisName(), this, &GameServerToWorldModule::OnObjectClassEvent);
...
void GameServerToWorldModule::SendOffline(const Guid& self)
{
	if (m_pKernelModule->ExistObject(self))
	{
		SquickStruct::RoleOfflineNotify xMsg;

		//const Guid& xClan = m_pKernelModule->GetPropertyObject(self, SquickProtocol::Player::Clan_ID());

		*xMsg.mutable_self() = INetModule::StructToProtobuf(self);
		*xMsg.mutable_clan() = INetModule::StructToProtobuf(Guid());
		xMsg.set_game(pPluginManager->GetAppID());
		xMsg.set_proxy(0);

		m_pNetClientModule->SendToAllServerByPB(SQUICK_SERVER_TYPES::SQUICK_ST_WORLD, SquickStruct::ACK_OFFLINE_NOTIFY, xMsg, self);
	}
}
