...
function load_script_file(pre_path, fileList, isReload)
	...
	if isReload == false then
		-- 注册完毕后直接执行一遍
		for i=1, #(fileList) do
			if fileList[i].tbl then
				if fileList[i].tbl.awake then
...
function module_awake(...)
	...
	if ScriptList then
		for i=1, #(ScriptList) do
			if ScriptList[i].tbl then
				if ScriptList[i].tbl.awake then
...
function module_after_init(...)
	...
	if ScriptList then
		for i=1, #(ScriptList) do
			if ScriptList[i].tbl then
				if ScriptList[i].tbl.after_init then
